...
from dataloader import OneShotIterator  #Bidirectional
...
def save_model(model, optimizer, save_variable_list, args):
    ...
    b = {'model_state_dict': model.state_dict(),
        'optimizer_state_dict': optimizer.state_dict()}
    b.update(save_variable_list,)
    torch.save(b,
...
def main(args):
    ...
    valid_triples = np.concatenate((hr, t_correct_index, t_candidate), axis=1)
    ...
    nentity = len(np.unique(train_triples[:, [0, 2]]))
    ...
    for i in tqdm(range(len(train_triples))):
        head, relation, tail = train_triples[i][0], train_triples[i][1], train_triples[i][2]
    ...
    if args.do_train:
        ...
        train_iterator = OneShotIterator(train_dataloader_tail)  #train_dataloader_head,   #Bidirectional
    ...
    if args.evaluate_train:
        ...
        small_train_triples = []
        indices = np.random.choice(len(train_triples), args.ntriples_eval_train, replace=False)
        for i in indices:
